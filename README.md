...
HashiCorp Vault — replace hardcoded secrets in properties files with dynamic secret injection
mTLS — mutual TLS for service-to-service authentication between gateway and resource server
...
JTI blacklisting — every JWT gets a unique ID (jti claim). The resource server checks Redis for blacklisted_jti:<jti> on every request. If the key exists, the request is rejected regardless of token validity.
User-level kill switch — on refresh token reuse detection, a revoked_user:<email> key is set in Redis. This blocks all requests from that user instantly, even tokens whose JTIs haven't been individually blacklisted yet.
